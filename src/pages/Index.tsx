  ...
  return (
    <article className="feed-reel group overflow-hidden rounded-[32px] bg-card shadow-[var(--shadow-editorial)] ring-1 ring-border/45 transition duration-200 active:scale-[0.99]">
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-secondary sm:aspect-[16/11]">
        <Link to={`/dish/${item.slug}`} className="absolute inset-0 block" aria-label={`View details for ${item.name}`} onPointerEnter={() => onImageIntent?.(item)} onPointerDown={() => onImageIntent?.(item)} onFocus={() => onImageIntent?.(item)}>
        ...
        <div className="absolute right-4 top-4 flex flex-col gap-3">
          <button type="button" className={cn("thumb-action save-pop size-14 bg-card/92", item.user_want_to_try && "bg-accent text-accent-foreground animate-scale-in")} onPointerDown={startLongPress} onPointerUp={clearLongPress} onPointerCancel={clearLongPress} onPointerLeave={clearLongPress} onClick={quickWantToTry} aria-label="Want to try. Hold to choose a list"><Bookmark className={cn("size-5", item.user_want_to_try && "fill-current")} /></button>
        ...
        <Link to={`/dish/${item.slug}`} className="absolute inset-x-0 bottom-0 block p-5 text-text-inverse sm:p-6">
          <div className="max-w-[calc(100%-1rem)] sm:max-w-[72%]">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 text-sm font-bold text-text-inverse/90"><span>{formatPrice(item)}</span><span>·</span><span>{item.review_count} reviews</span>{item.cuisine && <><span>·</span><span>{item.cuisine}</span></>}</div>
            {item.tags.length > 0 && <div className="mt-3 flex max-w-full gap-2 overflow-hidden">{item.tags.slice(0, 3).map((tag) => <span key={tag} className="inline-flex shrink-0 rounded-full bg-card/16 px-3 py-1 text-[11px] font-black text-text-inverse backdrop-blur-md ring-1 ring-card/20">{tag}</span>)}</div>}
          </div>
        </Link>
      </div>
    </article>
...
const ItemDetail = ({ item, transitionImageUrl, userLocation, sessionUser, onProtected, onSave, onDishAction, onReviewPublished, reviewRefreshKey }: { item: MenuItem; transitionImageUrl?: string | null; userLocation: { latitude: number; longitude: number } | null; sessionUser: UserSession; onProtected: (message: string) => void; onSave: (item: MenuItem) => void; onDishAction: (item: MenuItem, action: "want_to_try" | "favorite", enabled: boolean) => void; onReviewPublished: () => void; reviewRefreshKey: number }) => {
  ...
  return (
    <section className="-mx-3 -mt-3 max-w-[calc(100%+1.5rem)] space-y-0 overflow-visible lg:mx-0 lg:mt-0 lg:max-w-full">
      ...
      <div className="relative z-10 -mt-10 space-y-4 px-3 pb-6 lg:px-0">
...
const RelatedDishes = ({ tags, currentName }: { tags: string[]; currentName: string }) => {
  ...
  return <section className="max-w-full overflow-hidden rounded-3xl glass-surface p-4"><h2 className="font-display text-2xl font-black">You might also like</h2><div className="mt-3 grid w-full max-w-full min-w-0 grid-cols-1 gap-2 lg:grid-cols-1">{suggestions.map((tag) => <a key={tag} href={`/search?q=${encodeURIComponent(tag)}`} className="block w-full min-w-0 max-w-full overflow-hidden rounded-2xl bg-secondary/70 p-3 transition active:scale-95"><p className="text-sm font-black line-clamp-1">{tag}</p><p className="mt-1 text-xs font-bold text-text-secondary">Explore dishes</p></a>)}</div></section>;
};
...
const DishDetailLoader = () => <section className="overflow-hidden rounded-[30px] bg-card shadow-[var(--shadow-soft)] ring-1 ring-border/55"><div className="h-[58vh] min-h-96 animate-pulse bg-secondary" /><div className="space-y-3 p-5"><div className="h-8 w-3/4 animate-pulse rounded-full bg-muted" /><div className="h-4 w-1/2 animate-pulse rounded-full bg-muted" /><div className="grid grid-cols-3 gap-2 pt-2"><span className="h-20 animate-pulse rounded-2xl bg-secondary" /><span className="h-20 animate-pulse rounded-2xl bg-secondary" /><span className="h-20 animate-pulse rounded-2xl bg-secondary" /></div></div></section>;
...
const FeedEndCard = forwardRef<HTMLDivElement, { loadingMore: boolean; hasMoreItems: boolean; onAddPost: () => void; className?: string }>(({ loadingMore, hasMoreItems, onAddPost, className }, ref) => {
  if (loadingMore || hasMoreItems) return <div ref={ref} className={cn("flex min-h-20 items-center justify-center rounded-3xl border border-dashed bg-card/55 p-4 text-sm font-bold text-text-secondary", className)}>{loadingMore ? <><Loader2 className="mr-2 size-4 animate-spin text-primary" />Loading…</> : "Scroll for more"}</div>;
  return <section ref={ref} className={cn("relative isolate min-h-72 overflow-hidden rounded-[30px] border bg-card p-5 text-center shadow-[var(--shadow-soft)] ring-1 ring-border/50", className)}><div className="absolute inset-x-8 top-5 h-28 rounded-full bg-primary/10 blur-3xl" /><div className="relative mx-auto flex size-28 items-center justify-center"><div className="absolute inset-0 animate-pulse rounded-full bg-accent/20" /><div className="absolute size-20 animate-[spin_9s_linear_infinite] rounded-full border border-dashed border-primary/35" /><div className="relative flex size-16 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-[var(--shadow-editorial)]"><CameraIcon className="size-7" /></div><Sparkles className="absolute right-1 top-4 size-5 animate-scale-in text-primary" /><Star className="absolute bottom-3 left-1 size-4 animate-pulse fill-current text-accent" /></div><p className="mt-2 text-xs font-black uppercase tracking-[0.2em] text-primary">You’re caught up</p><h2 className="mx-auto mt-2 max-w-xs font-display text-3xl font-black leading-none">Your next great bite belongs here.</h2><p className="mx-auto mt-2 max-w-sm text-sm font-bold text-text-secondary">Snap a plate, add a quick rating, and help the feed discover what’s worth ordering next.</p><Button className="mt-5 rounded-full" onClick={onAddPost}><CameraIcon />Add your post</Button></section>;
});
FeedEndCard.displayName = "FeedEndCard";
...
const RestaurantDirectory = ({ restaurants, loading }: { restaurants: Restaurant[]; loading: boolean }) => {
  ...
  return <section className="space-y-3 rounded-lg border bg-card p-4 shadow-[var(--shadow-soft)]"><div><h2 className="font-display text-3xl font-black">Nearby restaurants from Google Maps</h2><p className="text-sm text-text-secondary">Places within 50 miles of your location. Add the first dish review when something looks good.</p></div><div className="grid gap-3 md:grid-cols-2">{restaurants.map((restaurant) => <article key={restaurant.google_place_id ?? restaurant.id ?? restaurant.name} className="rounded-md border bg-background p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-display text-xl font-black">{restaurant.name}</h3><p className="mt-1 text-sm text-text-secondary">{restaurant.address || restaurant.city || "Nearby"}</p></div>{restaurant.rating ? <span className="rounded-full bg-accent px-3 py-1 text-sm font-black text-primary-foreground"><Star className="mr-1 inline size-4 fill-current" />{restaurant.rating}</span> : null}</div><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" asChild><a href={`/search?q=${encodeURIComponent(restaurant.name)}`}>Be first to review this!</a></Button>{restaurant.maps_url && <Button size="sm" variant="outline" asChild><a href={restaurant.maps_url} target="_blank" rel="noreferrer"><Navigation />Maps</a></Button>}{restaurant.website_url && <Button size="sm" variant="outline" asChild><a href={restaurant.website_url} target="_blank" rel="noreferrer"><Globe />Website</a></Button>}</div></article>)}</div></section>;
};
        ...
        return (
          <button
            ...
            className={cn(
              ...
              fillPercent ? "text-primary drop-shadow-sm" : "text-text-secondary/35 hover:text-primary",
            )}
...
const ReviewFeed = ({ item, refreshKey }: { item: MenuItem; refreshKey: number }) => {
  ...
  const rows = reviews;
  return <section className="max-w-full space-y-3 overflow-hidden rounded-3xl glass-surface p-4"><h2 className="font-display text-3xl font-black">Reviews</h2>{rows.length ? rows.map((review) => <article key={review.id} className="max-w-full overflow-hidden rounded-2xl bg-secondary/55 p-3"><p className="font-bold"><span className="text-primary">{"★".repeat(Math.round(review.rating))}</span> {review.would_order_again ? "· would order again" : ""}</p>{review.price_paid ? <p className="text-xs font-bold text-primary">Paid ${review.price_paid} {review.currency}</p> : null}<div className="mt-2 grid min-w-0 grid-cols-1 gap-2 text-xs font-bold text-text-secondary sm:grid-cols-2 md:grid-cols-4"><span className="min-w-0 truncate">Temp {review.temperature_rating ?? "—"}/5</span><span className="min-w-0 truncate">Spice {review.spiciness_rating ?? "—"}/5</span><span className="min-w-0 truncate">Sweet {review.sweet_savory_rating ?? "—"}/5</span><span className="min-w-0 truncate">Flavor {review.flavor_intensity_rating ?? "—"}/5</span></div>{review.review && <p className="mt-2 text-sm text-text-secondary">{review.review}</p>}</article>) : <p className="text-sm font-semibold text-text-secondary">No reviews yet.</p>}</section>;
};
...
const WantToTryPlanner = ({ dishes, hasLocation, onUseLocation }: { dishes: WantToTryDish[]; hasLocation: boolean; onUseLocation: () => void }) => {
  ...
  return <section className="max-w-full overflow-hidden rounded-[28px] glass-surface p-4"><div className="mb-3 flex min-w-0 flex-wrap items-center justify-between gap-3"><div><h2 className="font-display text-2xl font-black">Want to try</h2>{nearest && <p className="text-xs font-bold text-text-secondary">Nearest: {nearest.name} · {nearest.distance_miles?.toFixed(1)} mi</p>}</div><div className="flex items-center gap-2"><span className="soft-chip">{dishes.length}</span>{!hasLocation && <Button size="sm" variant="outline" className="rounded-full" onClick={onUseLocation}><LocateFixed className="size-4" />Sort nearby</Button>}</div></div>{dishes.length ? <div className="space-y-4">{groups.map(([location, items]) => <div key={location} className="rounded-3xl bg-secondary/55 p-3"><div className="mb-3 flex min-w-0 items-center justify-between gap-2"><h3 className="min-w-0 break-words font-display text-xl font-black"><MapPin className="mr-1 inline size-4 text-primary" />{location}</h3><span className="soft-chip text-primary">{items.length}</span></div><div className="grid min-w-0 gap-3 md:grid-cols-2">{items.map((dish) => <a key={dish.id} href={`/dish/${dish.slug}`} className="group grid min-w-0 grid-cols-[72px_minmax(0,1fr)] gap-3 rounded-2xl bg-card/95 p-2 ring-1 ring-border/45 transition active:scale-[0.98] sm:grid-cols-[84px_minmax(0,1fr)]"><div className="aspect-square overflow-hidden rounded-2xl bg-secondary">{dish.cover_image_url ? <img src={dish.cover_image_url} alt={dish.name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" draggable={false} loading="lazy" decoding="async" sizes="(min-width: 768px) 33vw, 50vw" /> : <div className="flex h-full items-center justify-center"><ChefHat className="size-8 opacity-40" /></div>}</div><div className="min-w-0 py-1"><div className="mb-1 flex min-w-0 flex-wrap gap-1"><span className="soft-chip text-primary"><Star className="size-3 fill-current" />{Number(dish.aggregate_rating ?? 0).toFixed(1)}</span>{dish.distance_miles != null && <span className="soft-chip">{dish.distance_miles.toFixed(1)} mi</span>}</div><p className="line-clamp-2 font-display text-lg font-black leading-none">{dish.name}</p><p className="mt-1 line-clamp-1 text-xs font-bold text-text-secondary">{dish.restaurants?.name || dish.cuisine || "Dish"}</p><p className="mt-2 text-xs font-black text-primary">{dish.plan_group}</p></div></a>)}</div></div>)}</div> : <p className="rounded-3xl bg-secondary/60 p-4 text-sm font-bold text-text-secondary">Save dishes you are craving next.</p>}</section>;
};
