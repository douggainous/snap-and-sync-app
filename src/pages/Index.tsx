    ...
    <section className="-mx-3 space-y-4 md:mx-0">
      <div className="relative min-h-[calc(100svh-108px)] overflow-hidden bg-secondary shadow-[var(--shadow-editorial)] ring-1 ring-border/60 md:min-h-[760px] md:rounded-[32px]">
        <div className="flex h-full snap-x snap-mandatory overflow-x-auto">
          {[item.cover_image_url].filter(Boolean).map((photo) => <img key={photo} src={photo!} alt={`${item.name} menu item`} className="h-[calc(100svh-108px)] min-h-[620px] w-full shrink-0 snap-center object-cover md:h-[760px]" width={1024} height={1280} />)}
        ...
        <div className="absolute inset-x-0 bottom-0 space-y-4 p-4 pb-6 md:p-7">
          ...
          <div className="flex items-end justify-between gap-4"><div><p className="font-display text-5xl font-black text-accent">{item.aggregate_rating.toFixed(1)}<span className="text-2xl">★</span></p><p className="text-xs font-bold text-foreground/70">{item.review_count} reviews · {formatPrice(item)}</p></div><div className="flex gap-2"><button type="button" className={cn("thumb-action", item.user_favorite && "bg-primary text-primary-foreground")} onClick={() => onDishAction(item, "favorite", !item.user_favorite)} aria-label="Save dish"><Heart className={cn("size-5", item.user_favorite && "fill-current")} /></button><button type="button" className={cn("thumb-action", item.user_want_to_try && "bg-accent text-accent-foreground")} onClick={() => onDishAction(item, "want_to_try", !item.user_want_to_try)} aria-label="Want to try"><Bookmark className={cn("size-5", item.user_want_to_try && "fill-current")} /></button></div></div>
