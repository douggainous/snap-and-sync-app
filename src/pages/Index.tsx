  ...
  return (
    <article className="feed-reel group relative -mx-3 min-h-[calc(100svh-148px)] overflow-hidden bg-secondary shadow-[var(--shadow-editorial)] ring-1 ring-border/55 md:mx-0 md:w-full md:max-w-full md:min-h-[760px] md:rounded-[32px]">
      ...
      <div className="absolute bottom-0 left-0 right-16 p-4 pb-7 text-foreground sm:p-7">
        <p className="pointer-events-none mb-2 inline-flex max-w-full items-center gap-1 rounded-full bg-foreground/82 text-text-inverse px-3 py-1 text-[11px] font-black backdrop-blur-md"><MapPin className="size-3 shrink-0 text-primary" /><span className="truncate">{item.restaurants?.name ?? "Standalone dish"}{miles ? ` · ${miles.toFixed(1)} mi` : item.restaurants?.city ? ` · ${item.restaurants.city}` : ""}</span></p>
