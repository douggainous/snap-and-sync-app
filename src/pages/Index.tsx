    ...
    <main className="min-h-screen bg-background pb-28 text-foreground md:pb-8">
      ...
      <section className="mx-auto grid max-w-5xl gap-5 px-0 py-3 md:grid-cols-[180px_1fr] md:px-6 md:py-6">
        ...
        <div className="min-w-0 space-y-4 px-3 md:px-0">
            ...
            <>
              <section className="sticky top-[66px] z-20 -mx-3 border-y border-border/40 bg-background/62 px-3 py-2 backdrop-blur-2xl md:top-20 md:mx-0 md:rounded-full md:border md:px-2">
                ...
                <div className="sr-only">
                  <Select value={cuisineFilter} onValueChange={setCuisineFilter}><SelectTrigger><SelectValue placeholder="Cuisine" /></SelectTrigger><SelectContent><SelectItem value="all">All cuisines</SelectItem><SelectItem value="American">American</SelectItem><SelectItem value="Italian">Italian</SelectItem><SelectItem value="Japanese">Japanese</SelectItem><SelectItem value="Mexican">Mexican</SelectItem><SelectItem value="Thai">Thai</SelectItem><SelectItem value="Dessert">Dessert</SelectItem></SelectContent></Select>
                  <Select value={minRating} onValueChange={setMinRating}><SelectTrigger><SelectValue placeholder="Rating" /></SelectTrigger><SelectContent><SelectItem value="0">Any rating</SelectItem><SelectItem value="3.5">3.5★+</SelectItem><SelectItem value="4">4★+</SelectItem><SelectItem value="4.5">4.5★+</SelectItem></SelectContent></Select>
                </div>
              </section>
          ...
          {view === "scan" && (
            <section className="space-y-4 rounded-3xl glass-surface p-4 md:p-5">
              <div><h1 className="font-display text-4xl font-black leading-[0.95]">Snap a plate.</h1></div>
              <div className="overflow-hidden rounded-[28px] bg-secondary">{photoPreviews.length ? <div className="grid gap-2 p-2 sm:grid-cols-2 lg:grid-cols-3">{photoPreviews.map((photo, index) => <div key={photo} className="relative aspect-[4/5] overflow-hidden rounded-3xl bg-background"><img src={photo} alt={`Food review photo ${index + 1}`} className="h-full w-full object-cover" /><div className="absolute inset-x-3 top-3 flex justify-between gap-2"><span className="rounded-full bg-background/78 px-3 py-1 text-xs font-black text-foreground backdrop-blur-md">{index + 1}/{photoPreviews.length}</span><Button type="button" size="icon" variant="secondary" className="size-9 rounded-full" onClick={() => removeReviewPhoto(index)} aria-label="Remove photo"><X className="size-4" /></Button></div></div>)}</div> : <button onClick={captureReviewPhoto} className="flex h-[62vh] min-h-96 w-full flex-col items-center justify-center gap-3 text-muted-foreground"><span className="flex size-24 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-[var(--shadow-soft)]"><CameraIcon className="size-11" /></span><span className="font-display text-3xl font-black text-foreground">Take photo</span></button>}</div>
              <div className="grid gap-3 md:grid-cols-2"><Input className="h-12 rounded-full bg-secondary/80" placeholder="Dish name" value={scanDish} onChange={(event) => setScanDish(event.target.value)} /><Input className="h-12 rounded-full bg-secondary/80" placeholder="Restaurant optional" value={scanRestaurant} onChange={(event) => setScanRestaurant(event.target.value)} /></div>
              ...
              <div className="sticky bottom-24 z-10 grid gap-2 sm:grid-cols-3"><Button type="button" className="h-12 rounded-full" onClick={captureReviewPhoto}><CameraIcon />{photoPreviews.length ? "Add" : "Camera"}</Button><Button type="button" className="h-12 rounded-full" variant="outline" onClick={selectPhotos}><Upload />Library</Button><Button type="button" className="h-12 rounded-full" onClick={startPhotoReview} disabled={!imageFiles.length}><Star />Review</Button></div>
      ...
      <nav className="fixed inset-x-3 bottom-3 z-20 grid grid-cols-5 items-center rounded-full glass-surface p-1.5 md:hidden">
        {navItems.slice(0, 2).map((item) => <button key={item.id} onClick={() => { setSearchPanelOpen(false); setView(item.id); if (item.id !== "discover") navigate("/"); }} className={cn("flex h-12 flex-col items-center justify-center gap-0.5 rounded-full px-1 text-[10px] font-bold text-muted-foreground", view === item.id && "bg-primary text-primary-foreground")}><item.icon className="size-5" />{item.label}</button>)}
        <button onClick={() => setSearchPanelOpen((open) => !open)} className={cn("mx-auto flex size-14 items-center justify-center rounded-full bg-accent text-accent-foreground shadow-[var(--shadow-editorial)] transition active:scale-95", searchPanelOpen && "bg-primary text-primary-foreground")} aria-label="Open search"><Search className="size-6" /></button>
  ...
  return (
    <section className="space-y-4">
      <div className="overflow-hidden rounded-[28px] bg-card shadow-[var(--shadow-editorial)] ring-1 ring-border/60">
        <div className="relative min-h-[62vh] overflow-hidden bg-secondary">
          {item.cover_image_url ? <img src={item.cover_image_url} alt={`${item.name} menu item`} className="absolute inset-0 h-full w-full object-cover" width={1024} height={1280} /> : <div className="absolute inset-0 flex items-center justify-center bg-secondary"><ChefHat className="size-20 opacity-40" /></div>}
          <div className="absolute inset-0 bg-gradient-to-t from-background via-background/20 to-transparent" />
          <div className="absolute bottom-0 left-0 right-0 p-5"><p className="soft-chip mb-3 text-accent">{item.cuisine || item.section || "Dish"}</p><h1 className="font-display text-5xl font-black leading-[0.92]">{item.name}</h1>{item.description && <p className="mt-2 line-clamp-2 text-sm font-semibold text-foreground/75">{item.description}</p>}</div>
        </div>
        <div className="space-y-4 p-4"><div className="grid grid-cols-4 gap-2"><Metric icon={Star} label="rating" value={`${item.aggregate_rating}★`} /><Metric icon={Clock} label="reviews" value={String(item.review_count)} /><Metric icon={MapPin} label="miles" value={miles ? `${miles.toFixed(1)}` : "—"} /><Metric icon={Bookmark} label="price" value={formatPrice(item)} /></div><div className="grid grid-cols-3 gap-2"><Button className="rounded-full" onClick={() => document.getElementById("review-menu-item")?.scrollIntoView({ behavior: "smooth", block: "start" })}><Star />Review</Button><Button className="rounded-full" variant="outline" onClick={() => onSave(item)}><Bookmark />Save</Button><Button className="rounded-full" variant="outline" onClick={shareItem}><Share2 />Share</Button></div></div>
      </div>
      <div className="grid gap-4 lg:grid-cols-[1fr_300px]"><div className="space-y-4"><ReviewForm item={item} sessionUser={sessionUser} onProtected={onProtected} onPublished={onReviewPublished} /><ReviewFeed item={item} refreshKey={reviewRefreshKey} /></div><div className="rounded-3xl glass-surface p-4"><h2 className="font-display text-2xl font-black">Place</h2><p className="mt-2 font-bold">{item.restaurants?.name}</p><p className="text-sm text-muted-foreground">{item.restaurants?.address} · {item.restaurants?.city}</p><div className="mt-3 grid gap-2"><Button className="w-full rounded-full" asChild><a href={mapsDirectionsUrl(item.restaurants, "driving")} target="_blank" rel="noreferrer"><Navigation />Drive</a></Button><Button className="w-full rounded-full" variant="outline" asChild><a href={mapsDirectionsUrl(item.restaurants, "walking")} target="_blank" rel="noreferrer"><Footprints />Walk</a></Button>{callUrl && <Button className="w-full rounded-full" variant="outline" asChild><a href={callUrl}><Phone />Call</a></Button>}{webUrl && <Button className="w-full rounded-full" variant="outline" asChild><a href={webUrl} target="_blank" rel="noreferrer"><Globe />Website</a></Button>}{mailUrl && <Button className="w-full rounded-full" variant="outline" asChild><a href={mailUrl}><Mail />Email</a></Button>}</div></div></div>
    </section>
  );
};
...
const PhotoReviewComposer = ({ imageFiles, photoPreviews, restaurantName, dishName, onRestaurantNameChange, onDishNameChange, sessionUser, onProtected, onPublished }: { imageFiles: File[]; photoPreviews: string[]; restaurantName: string; dishName: string; onRestaurantNameChange: (value: string) => void; onDishNameChange: (value: string) => void; sessionUser: UserSession; onProtected: (message: string) => void; onPublished: () => void }) => {
  ...
  return <form id="photo-review-form" onSubmit={publishPhotoReview} className="mt-5 space-y-4 rounded-3xl bg-background/55 p-4"><h2 className="font-display text-3xl font-black">Create dish</h2><div className="grid gap-3 md:grid-cols-2"><label className="text-sm font-bold">Dish<Input className="mt-1 h-12 rounded-full bg-secondary/80" value={dishName} onChange={(event) => onDishNameChange(event.target.value)} maxLength={120} placeholder="Dish name" required /></label><label className="text-sm font-bold">Restaurant<Input className="mt-1 h-12 rounded-full bg-secondary/80" value={restaurantName} onChange={(event) => onRestaurantNameChange(event.target.value)} maxLength={120} placeholder="Optional" /></label></div><div className="rounded-2xl bg-secondary/70 p-3"><p className="mb-2 text-sm font-black">Rating</p><StarRating value={rating} onChange={setRating} /></div><div className="grid gap-3 md:grid-cols-2"><QuickScale label="Temp" low="cold" high="hot" value={temperature} onChange={setTemperature} /><QuickScale label="Spice" low="none" high="fire" value={spiciness} onChange={setSpiciness} min={0} /><QuickScale label="Sweet ↔ savory" low="sweet" high="savory" value={sweetSavory} onChange={setSweetSavory} /><QuickScale label="Flavor" low="subtle" high="bold" value={flavorIntensity} onChange={setFlavorIntensity} /></div><div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-bold">Price<Input className="mt-1 h-12 rounded-full bg-secondary/80" type="number" min="0" max="10000" step="0.01" value={pricePaid} onChange={(event) => setPricePaid(event.target.value)} placeholder="Optional" /></label><label className="flex items-center gap-2 rounded-2xl bg-secondary/70 p-3 text-sm font-bold"><input type="checkbox" checked={wouldOrderAgain} onChange={(event) => setWouldOrderAgain(event.target.checked)} />Would order again</label></div><Textarea className="rounded-2xl bg-secondary/80" value={review} onChange={(event) => setReview(event.target.value)} maxLength={1200} placeholder="Optional note" /><Input className="h-12 rounded-full bg-secondary/80" value={tags} onChange={(event) => setTags(event.target.value)} maxLength={140} placeholder="Tags: crispy, spicy" /><Button type="submit" className="h-12 w-full rounded-full" disabled={saving}>{saving ? <Loader2 className="animate-spin" /> : <Sparkles />}Save dish</Button></form>;
};
...
const ReviewForm = ({ item, sessionUser, onProtected, onPublished }: { item: MenuItem; sessionUser: UserSession; onProtected: (message: string) => void; onPublished: () => void }) => {
  ...
  return <section id="review-menu-item" className="rounded-3xl glass-surface p-4"><h2 className="font-display text-3xl font-black">Rate it</h2><form onSubmit={publishReview} className="mt-4 space-y-4"><div className="rounded-2xl bg-secondary/70 p-3"><p className="mb-2 text-sm font-black">Rating</p><StarRating value={rating} onChange={setRating} /></div><div className="grid gap-3 md:grid-cols-2"><QuickScale label="Temp" low="cold" high="hot" value={temperature} onChange={setTemperature} /><QuickScale label="Spice" low="none" high="fire" value={spiciness} onChange={setSpiciness} min={0} /><QuickScale label="Sweet ↔ savory" low="sweet" high="savory" value={sweetSavory} onChange={setSweetSavory} /><QuickScale label="Flavor" low="subtle" high="bold" value={flavorIntensity} onChange={setFlavorIntensity} /></div><div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-bold">Price<Input className="mt-1 h-12 rounded-full bg-secondary/80" type="number" min="0" max="10000" step="0.01" value={pricePaid} onChange={(event) => setPricePaid(event.target.value)} placeholder="Optional" /></label><label className="flex items-center gap-2 rounded-2xl bg-secondary/70 p-3 text-sm font-bold"><input type="checkbox" checked={wouldOrderAgain} onChange={(event) => setWouldOrderAgain(event.target.checked)} />Would order again</label></div><Textarea className="rounded-2xl bg-secondary/80" value={review} onChange={(event) => setReview(event.target.value)} maxLength={1200} placeholder={`Optional note about ${item.name}`} /><Input className="h-12 rounded-full bg-secondary/80" value={tags} onChange={(event) => setTags(event.target.value)} maxLength={140} placeholder="Tags: crispy, spicy" /><Button type="submit" className="h-12 rounded-full" disabled={saving}>{saving ? <Loader2 className="animate-spin" /> : <Star />}Submit</Button></form></section>;
};
...
const ReviewFeed = ({ item, refreshKey }: { item: MenuItem; refreshKey: number }) => {
  ...
  const rows = reviews;
  return <section className="space-y-3 rounded-3xl glass-surface p-4"><h2 className="font-display text-3xl font-black">Reviews</h2>{rows.length ? rows.map((review) => <article key={review.id} className="rounded-2xl bg-secondary/55 p-3"><p className="font-bold"><span className="text-accent">{"★".repeat(Math.round(review.rating))}</span> {review.would_order_again ? "· would order again" : ""}</p>{review.price_paid ? <p className="text-xs font-bold text-accent">Paid ${review.price_paid} {review.currency}</p> : null}<div className="mt-2 grid grid-cols-2 gap-2 text-xs font-bold text-muted-foreground md:grid-cols-4"><span>Temp {review.temperature_rating ?? "—"}/5</span><span>Spice {review.spiciness_rating ?? "—"}/5</span><span>Sweet {review.sweet_savory_rating ?? "—"}/5</span><span>Flavor {review.flavor_intensity_rating ?? "—"}/5</span></div>{review.review && <p className="mt-2 text-sm text-muted-foreground">{review.review}</p>}</article>) : <p className="text-sm font-semibold text-muted-foreground">No reviews yet.</p>}</section>;
};
