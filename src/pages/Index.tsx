...
const EmotionalRating = ({ value, onChange, loved, onLovedChange, wantToTry, onWantToTryChange }: { value: number; onChange: (value: number) => void; loved: boolean; onLovedChange: (value: boolean) => void; wantToTry: boolean; onWantToTryChange: (value: boolean) => void }) => {
  ...
  return <div className="space-y-3"><div className="grid grid-cols-4 gap-2">{moods.map((mood) => <button key={mood.label} type="button" onClick={() => onChange(mood.score)} className={cn("group flex min-h-24 flex-col items-center justify-center rounded-[26px] border bg-secondary/70 p-2 text-center shadow-[var(--shadow-soft)] transition active:scale-95", value === mood.score ? "border-accent bg-accent text-accent-foreground" : "border-foreground/10 hover:border-primary/60 hover:bg-secondary")} aria-pressed={value === mood.score}><span className="text-3xl transition group-active:scale-125">{mood.emoji}</span><span className="mt-2 text-xs font-black">{mood.label}</span></button>)}</div><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => { onLovedChange(!loved); if (!loved) onChange(5); }} className={cn("flex h-14 items-center justify-center gap-2 rounded-full border text-sm font-black transition active:scale-95", loved ? "border-primary bg-primary text-primary-foreground" : "border-foreground/10 bg-secondary/70 text-foreground")}><Heart className={cn("size-5", loved && "fill-current")} />Loved it</button><button type="button" onClick={() => onWantToTryChange(!wantToTry)} className={cn("flex h-14 items-center justify-center gap-2 rounded-full border text-sm font-black transition active:scale-95", wantToTry ? "border-accent bg-accent text-accent-foreground" : "border-foreground/10 bg-secondary/70 text-foreground")}><Bookmark className={cn("size-5", wantToTry && "fill-current")} />Want to try</button></div><div className="flex items-center justify-center gap-1 text-accent" aria-hidden="true">{[1, 2, 3, 4, 5].map((star) => <Star key={star} className={cn("size-6 transition", value >= star ? "fill-current scale-110" : "opacity-25")} />)}</div></div>;
};
...
const PhotoReviewComposer = ({ imageFiles, photoPreviews, restaurantName, dishName, onRestaurantNameChange, onDishNameChange, sessionUser, onProtected, onPublished }: { imageFiles: File[]; photoPreviews: string[]; restaurantName: string; dishName: string; onRestaurantNameChange: (value: string) => void; onDishNameChange: (value: string) => void; sessionUser: UserSession; onProtected: (message: string) => void; onPublished: () => void }) => {
  ...
  return <form id="photo-review-form" onSubmit={publishPhotoReview} className="space-y-3 px-3 animate-fade-in md:px-0"><div className="rounded-[28px] glass-surface p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><p className="soft-chip text-accent"><Sparkles className="size-4" />AI suggestions</p><h2 className="mt-2 font-display text-3xl font-black leading-none">Rate fast</h2></div><span className="rounded-full bg-primary px-3 py-1 text-sm font-black text-primary-foreground">{rating}★</span></div><div className="rounded-3xl bg-secondary/70 p-3"><StarRating value={rating} onChange={setRating} /></div></div><div className="grid gap-2 rounded-[28px] glass-surface p-3 md:grid-cols-2"><Input className="h-12 rounded-full bg-secondary/80 px-5 font-black" value={dishName} onChange={(event) => onDishNameChange(event.target.value)} maxLength={120} placeholder="Dish name" required /><Input className="h-12 rounded-full bg-secondary/80 px-5 font-bold" value={restaurantName} onChange={(event) => onRestaurantNameChange(event.target.value)} maxLength={120} placeholder="Restaurant optional" /><Input className="h-12 rounded-full bg-secondary/80 px-5" type="number" min="0" max="10000" step="0.01" value={pricePaid} onChange={(event) => setPricePaid(event.target.value)} placeholder="Price optional" /><label className="flex h-12 items-center justify-between rounded-full bg-secondary/80 px-5 text-sm font-black"><span>Order again</span><input type="checkbox" checked={wouldOrderAgain} onChange={(event) => setWouldOrderAgain(event.target.checked)} /></label></div><div className="grid gap-2 px-3 md:grid-cols-2 md:px-0"><QuickScale label="Temp" low="cold" high="hot" value={temperature} onChange={setTemperature} /><QuickScale label="Spice" low="none" high="fire" value={spiciness} onChange={setSpiciness} min={0} /><QuickScale label="Sweet ↔ savory" low="sweet" high="savory" value={sweetSavory} onChange={setSweetSavory} /><QuickScale label="Flavor" low="subtle" high="bold" value={flavorIntensity} onChange={setFlavorIntensity} /></div><div className="space-y-2 px-3 md:px-0"><Textarea className="min-h-20 rounded-3xl bg-secondary/80" value={review} onChange={(event) => setReview(event.target.value)} maxLength={1200} placeholder="Optional note" /><Input className="h-12 rounded-full bg-secondary/80" value={tags} onChange={(event) => setTags(event.target.value)} maxLength={140} placeholder="Tags: crispy, spicy" /></div><Button type="submit" className="sticky bottom-24 z-10 h-14 w-full rounded-full text-base shadow-[var(--shadow-editorial)]" disabled={saving}>{saving ? <Loader2 className="animate-spin" /> : <Sparkles />}Save dish</Button></form>;
};
