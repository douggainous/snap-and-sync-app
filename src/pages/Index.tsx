  ...
  return (
    <article className="feed-reel group relative -mx-3 min-h-[calc(100svh-148px)] overflow-hidden bg-secondary shadow-[var(--shadow-editorial)] ring-1 ring-border/55 md:mx-0 md:w-full md:max-w-full md:min-h-[760px] md:rounded-[32px]">
      ...
      <div className="absolute bottom-0 left-0 right-16 p-4 pb-7 text-foreground sm:p-7">
        <p className="pointer-events-none mb-2 inline-flex max-w-full items-center gap-1 rounded-full bg-foreground/82 text-text-inverse px-3 py-1 text-[11px] font-black text-foreground backdrop-blur-md"><MapPin className="size-3 shrink-0 text-primary" /><span className="truncate">{item.restaurants?.name ?? "Standalone dish"}{miles ? ` · ${miles.toFixed(1)} mi` : item.restaurants?.city ? ` · ${item.restaurants.city}` : ""}</span></p>
      ...
      <div className="absolute bottom-6 right-3 z-10 flex flex-col gap-3">
        ...
        <button type="button" className={cn("thumb-action save-pop", item.user_want_to_try && "bg-accent text-primary-foreground animate-scale-in")} onClick={(event) => { event.preventDefault(); onDishAction?.(item, "want_to_try", !item.user_want_to_try); }} aria-label="Want to try"><Bookmark className={cn("size-5", item.user_want_to_try && "fill-current")} /></button>
    ...
    <main className="min-h-screen w-full max-w-full overflow-x-hidden bg-background pb-28 text-foreground md:pb-8">
      ...
      <header className="sticky top-0 z-30 w-full max-w-full border-b border-border/50 bg-card/95 backdrop-blur-2xl">
        <div className="mx-auto flex w-full max-w-5xl items-center gap-2 px-4 py-3 md:gap-3 md:px-6">
          ...
          <form onSubmit={submitSearch} className="relative ml-auto hidden flex-1 md:block md:max-w-xl"><Search className="absolute left-4 top-1/2 size-4 -translate-y-1/2 text-secondary" /><Input className="h-11 rounded-full border-border/70 bg-secondary/70 pl-10 pr-12" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search dishes" /><button type="button" onClick={askLocation} className="absolute right-1 top-1/2 inline-flex size-9 -translate-y-1/2 items-center justify-center rounded-full text-secondary transition hover:bg-card hover:text-foreground" aria-label="Use my location"><LocateFixed className="size-4" /></button></form>
          ...
          {view === "discover" && !selectedItem && !listSlug && (
            <>
              <section className="relative z-10 -mx-3 mb-3 max-w-[calc(100%+1.5rem)] space-y-3 overflow-hidden border-b border-border/40 bg-background px-3 pb-3 pt-2 backdrop-blur-2xl lg:sticky lg:top-20 lg:mx-0 lg:max-w-full lg:rounded-[28px] lg:border lg:bg-card/95 lg:py-3">
                <form onSubmit={submitSearch} className="relative"><Search className="absolute left-4 top-1/2 size-5 -translate-y-1/2 text-secondary" /><Input className="h-14 rounded-full border-foreground/10 bg-secondary/70 pl-12 pr-12 text-lg font-black" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search dishes" /><button type="button" onClick={askLocation} className="absolute right-2 top-1/2 inline-flex size-10 -translate-y-1/2 items-center justify-center rounded-full bg-card/95 text-secondary" aria-label="Use my location"><LocateFixed className="size-5" /></button></form>
              ...
              {!query && <div className="relative z-0 mt-1 flex max-w-full min-w-0 flex-wrap gap-2 overflow-hidden pb-1">{trendingQueries.map((trend) => <button key={trend} type="button" onClick={() => applySearchSuggestion(trend)} className="soft-chip shrink-0 text-primary"><Sparkles className="size-4" />{trend}</button>)}</div>}
              ...
              {loading ? <SearchResultsLoader /> : feedError ? <FeedErrorState message={feedError} onRetry={() => void loadItems(query, false, feedMode, userLocation)} /> : query ? <div className="grid min-w-0 grid-cols-2 gap-3 md:grid-cols-3">{displayedItems.length ? displayedItems.map((item) => <SearchDishCard key={item.id} item={item} userLocation={userLocation} onDishAction={toggleDishAction} />) : <div className="col-span-full rounded-3xl border border-dashed bg-card p-6 text-center"><ChefHat className="mx-auto mb-3 size-10 text-primary" /><h2 className="font-display text-2xl font-black">No dishes yet</h2><p className="text-sm text-secondary">Capture the first plate.</p><Button className="mt-4 rounded-full" onClick={() => setView("scan")}><CameraIcon />Add dish</Button></div>}<div ref={loadMoreRef} className="col-span-full flex min-h-20 items-center justify-center rounded-3xl border border-dashed bg-card/55 p-4 text-sm font-bold text-secondary">{loadingMore ? <><Loader2 className="mr-2 size-4 animate-spin text-primary" />Loading…</> : hasMoreItems ? "More dishes loading" : "You’re caught up"}</div></div> : <div className="feed-scroll min-w-0 max-w-full space-y-4 overflow-hidden">{displayedItems.length ? displayedItems.map((item) => <FeedItemCard key={item.id} item={item} userLocation={userLocation} onSave={setFavoriteTarget} onFirstReview={startFirstReview} onDishAction={toggleDishAction} />) : <div className="rounded-3xl border border-dashed bg-card p-6 text-center"><ChefHat className="mx-auto mb-3 size-10 text-primary" /><h2 className="font-display text-2xl font-black">No dishes yet</h2><p className="text-sm text-secondary">Capture the first plate.</p><Button className="mt-4 rounded-full" onClick={() => setView("scan")}><CameraIcon />Add dish</Button></div>}<div ref={loadMoreRef} className="flex min-h-20 items-center justify-center rounded-3xl border border-dashed bg-card/55 p-4 text-sm font-bold text-secondary">{loadingMore ? <><Loader2 className="mr-2 size-4 animate-spin text-primary" />Loading…</> : hasMoreItems ? "Scroll for more" : "You’re caught up"}</div></div>}
            </>
          )}
          ...
          {view === "scan" && (
            <section className="-mx-3 space-y-3 overflow-hidden md:mx-0">
              <div className="relative min-h-[calc(100svh-164px)] w-full max-w-full overflow-hidden bg-secondary shadow-[var(--shadow-editorial)] ring-1 ring-border/55 md:rounded-[32px]">
                {photoPreviews.length ? <img src={photoPreviews[0]} alt="Captured dish" className="absolute inset-0 h-full w-full object-cover animate-scale-in" decoding="async" /> : <button onClick={captureReviewPhoto} className="absolute inset-0 flex w-full flex-col items-center justify-center gap-6 bg-secondary text-foreground"><span className="flex size-32 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-[var(--shadow-editorial)] transition active:scale-95"><CameraIcon className="size-14" /></span><span className="soft-chip text-primary"><Sparkles className="size-4" />Point. Snap. Rate.</span></button>}
                <div className="absolute inset-0 pointer-events-none bg-gradient-to-t from-foreground/82 via-foreground/34 to-foreground/42" />
                <div className="absolute left-4 top-4 soft-chip text-primary"><CameraIcon className="size-4" />Capture</div>
                {photoPreviews.length > 0 && <div className="absolute right-4 top-4 flex gap-2"><span className="soft-chip text-primary"><Sparkles className="size-4" />AI ready</span><Button type="button" size="icon" variant="secondary" className="size-10 rounded-full bg-card/95 backdrop-blur-xl" onClick={() => removeReviewPhoto(0)} aria-label="Remove photo"><X className="size-4" /></Button></div>}
                <div className="absolute inset-x-0 bottom-0 p-4 pb-6">
                  {photoPreviews.length ? <div className="space-y-3 animate-fade-in"><div className="grid gap-2"><Input className="h-12 rounded-full border-foreground/10 bg-card/95 px-5 text-base font-black backdrop-blur-xl" placeholder="Dish name" value={scanDish} onChange={(event) => setScanDish(event.target.value)} /><Input className="h-12 rounded-full border-foreground/10 bg-card/95 px-5 text-base font-bold backdrop-blur-xl" placeholder="Restaurant optional" value={scanRestaurant} onChange={(event) => setScanRestaurant(event.target.value)} /></div></div> : <div className="flex items-center justify-center gap-3"><Button type="button" variant="outline" className="h-12 rounded-full bg-card/95 px-5 backdrop-blur-xl" onClick={selectPhotos}><Upload />Library</Button><button type="button" onClick={captureReviewPhoto} className="flex size-24 items-center justify-center rounded-full border-4 border-foreground/25 bg-primary text-primary-foreground shadow-[var(--shadow-editorial)] transition active:scale-90" aria-label="Take photo"><CameraIcon className="size-10" /></button></div>}
                </div>
              </div>
      ...
      <nav className="fixed bottom-3 left-3 right-3 z-20 grid min-w-0 grid-cols-5 items-center overflow-hidden rounded-full glass-surface p-1.5 lg:hidden">
        {navItems.slice(0, 2).map((item) => <button key={item.id} onClick={() => { setSearchPanelOpen(false); setView(item.id); if (item.id !== "discover") navigate("/"); }} className={cn("flex h-12 min-w-0 flex-col items-center justify-center gap-0.5 rounded-full px-1 text-[10px] font-bold text-secondary", view === item.id && "bg-primary text-primary-foreground")}><item.icon className="size-5" /><span className="truncate">{item.label}</span></button>)}
        <button onClick={() => setSearchPanelOpen((open) => !open)} className={cn("mx-auto flex size-12 items-center justify-center rounded-full bg-accent text-primary-foreground shadow-[var(--shadow-editorial)] transition active:scale-95 sm:size-14", searchPanelOpen && "bg-primary text-primary-foreground")} aria-label="Open search"><Search className="size-6" /></button>
...
const ItemDetail = ({ item, userLocation, sessionUser, onProtected, onSave, onDishAction, onReviewPublished, reviewRefreshKey }: { item: MenuItem; userLocation: { latitude: number; longitude: number } | null; sessionUser: UserSession; onProtected: (message: string) => void; onSave: (item: MenuItem) => void; onDishAction: (item: MenuItem, action: "want_to_try" | "favorite", enabled: boolean) => void; onReviewPublished: () => void; reviewRefreshKey: number }) => {
  ...
  return (
    <section className="max-w-full space-y-4 overflow-hidden">
      ...
      <div className="max-w-full space-y-4"><div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_300px]"><div className="space-y-4"><ReviewForm item={item} sessionUser={sessionUser} onProtected={onProtected} onPublished={onReviewPublished} /><ReviewFeed item={item} refreshKey={reviewRefreshKey} /></div><div className="space-y-4"><RelatedDishes tags={relatedSearches} currentName={item.name} /><div className="rounded-3xl glass-surface p-4"><h2 className="font-display text-2xl font-black">Place</h2><p className="mt-2 font-bold">{item.restaurants?.name}</p><p className="text-sm text-secondary">{[item.restaurants?.address, item.restaurants?.city, miles ? `${miles.toFixed(1)} mi` : null].filter(Boolean).join(" · ")}</p><div className="mt-3 grid gap-2"><Button className="w-full rounded-full" asChild><a href={mapsDirectionsUrl(item.restaurants, "driving")} target="_blank" rel="noreferrer"><Navigation />Drive</a></Button><Button className="w-full rounded-full" variant="outline" asChild><a href={mapsDirectionsUrl(item.restaurants, "walking")} target="_blank" rel="noreferrer"><Footprints />Walk</a></Button>{callUrl && <Button className="w-full rounded-full" variant="outline" asChild><a href={callUrl}><Phone />Call</a></Button>}{webUrl && <Button className="w-full rounded-full" variant="outline" asChild><a href={webUrl} target="_blank" rel="noreferrer"><Globe />Website</a></Button>}{mailUrl && <Button className="w-full rounded-full" variant="outline" asChild><a href={mailUrl}><Mail />Email</a></Button>}</div></div>{isUuid(item.id) && <RestaurantDishTools item={item} sessionUser={sessionUser} onProtected={onProtected} />}</div></div></div>
    </section>
  );
};

const Metric = ({ icon: Icon, label, value }: { icon: typeof Star; label: string; value: string }) => <div className="rounded-2xl bg-secondary/80 p-3"><Icon className="mb-2 size-5 text-primary" /><p className="font-display text-xl font-black">{value}</p><p className="text-[11px] font-bold text-secondary">{label}</p></div>;

const RelatedDishes = ({ tags, currentName }: { tags: string[]; currentName: string }) => {
  const suggestions = tags.length ? tags : [currentName];
  return <section className="max-w-full overflow-hidden rounded-3xl glass-surface p-4"><h2 className="font-display text-2xl font-black">You might also like</h2><div className="mt-3 grid w-full max-w-full min-w-0 grid-cols-1 gap-2 lg:grid-cols-1">{suggestions.map((tag) => <a key={tag} href={`/search?q=${encodeURIComponent(tag)}`} className="block w-full min-w-0 max-w-full overflow-hidden rounded-2xl bg-secondary/70 p-3 transition active:scale-95"><p className="text-sm font-black line-clamp-1">{tag}</p><p className="mt-1 text-xs font-bold text-secondary">Explore dishes</p></a>)}</div></section>;
};

const GuestConversionNudge = ({ onClose, onSignIn }: { onClose: () => void; onSignIn: () => void }) => <div className="fixed inset-x-3 bottom-20 z-20 mx-auto max-w-md rounded-lg border bg-card p-4 shadow-[var(--shadow-editorial)] lg:bottom-6"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-display text-xl font-black">Keep this dish handy</p><p className="mt-1 text-sm font-bold text-secondary">Save dishes you want to try and track your favorite meals.</p></div><Button size="icon" variant="ghost" className="shrink-0" onClick={onClose} aria-label="Dismiss"><X className="size-4" /></Button></div><div className="mt-3 flex gap-2"><Button className="flex-1 rounded-full" onClick={onSignIn}><Bookmark className="size-4" />Save later</Button><Button variant="outline" className="rounded-full" onClick={onClose}>Not now</Button></div></div>;

const FeedErrorState = ({ message, onRetry }: { message: string; onRetry: () => void }) => <section className="rounded-3xl border border-dashed bg-card p-6 text-center"><ChefHat className="mx-auto mb-3 size-10 text-primary" /><h2 className="font-display text-2xl font-black">Could not load dishes</h2><p className="mt-1 text-sm font-semibold text-secondary">{message}</p><Button className="mt-4 rounded-full" onClick={onRetry}>Retry</Button></section>;
...
const RestaurantDirectory = ({ restaurants, loading }: { restaurants: Restaurant[]; loading: boolean }) => {
  if (loading) return <section className="rounded-lg border bg-card p-4 shadow-[var(--shadow-soft)]"><div className="flex items-center gap-2 font-bold text-primary"><Loader2 className="size-4 animate-spin" />Loading Google Maps restaurants within 50 miles…</div></section>;
  if (!restaurants.length) return null;
  return <section className="space-y-3 rounded-lg border bg-card p-4 shadow-[var(--shadow-soft)]"><div><h2 className="font-display text-3xl font-black">Nearby restaurants from Google Maps</h2><p className="text-sm text-secondary">Places within 50 miles of your location. Add the first dish review when something looks good.</p></div><div className="grid gap-3 md:grid-cols-2">{restaurants.map((restaurant) => <article key={restaurant.google_place_id ?? restaurant.id ?? restaurant.name} className="rounded-md border bg-background p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-display text-xl font-black">{restaurant.name}</h3><p className="mt-1 text-sm text-secondary">{restaurant.address || restaurant.city || "Nearby"}</p></div>{restaurant.rating ? <span className="rounded-full bg-accent px-3 py-1 text-sm font-black text-primary-foreground"><Star className="mr-1 inline size-4 fill-current" />{restaurant.rating}</span> : null}</div><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" asChild><a href={`/search?q=${encodeURIComponent(restaurant.name)}`}>Be first to review this!</a></Button>{restaurant.maps_url && <Button size="sm" variant="outline" asChild><a href={restaurant.maps_url} target="_blank" rel="noreferrer"><Navigation />Maps</a></Button>}{restaurant.website_url && <Button size="sm" variant="outline" asChild><a href={restaurant.website_url} target="_blank" rel="noreferrer"><Globe />Website</a></Button>}</div></article>)}</div></section>;
};
        ...
        return (
          <button
            ...
            className={cn(
              ...
              fillPercent ? "text-primary drop-shadow-sm" : "text-secondary/35 hover:text-primary",
            )}
          ...
          >
            <span className="relative block size-10">
              ...
              <span className="absolute inset-0 overflow-hidden transition-all duration-200 ease-out" style={{ width: `${fillPercent}%` }}>
                <Star className="size-10 fill-current text-primary transition-transform duration-200 group-active:scale-125 group-hover:scale-110" />
              </span>
            </span>
          </button>
        );
...
const ReviewFeed = ({ item, refreshKey }: { item: MenuItem; refreshKey: number }) => {
  ...
  const rows = reviews;
  return <section className="max-w-full space-y-3 overflow-hidden rounded-3xl glass-surface p-4"><h2 className="font-display text-3xl font-black">Reviews</h2>{rows.length ? rows.map((review) => <article key={review.id} className="max-w-full overflow-hidden rounded-2xl bg-secondary/55 p-3"><p className="font-bold"><span className="text-primary">{"★".repeat(Math.round(review.rating))}</span> {review.would_order_again ? "· would order again" : ""}</p>{review.price_paid ? <p className="text-xs font-bold text-primary">Paid ${review.price_paid} {review.currency}</p> : null}<div className="mt-2 grid min-w-0 grid-cols-1 gap-2 text-xs font-bold text-secondary sm:grid-cols-2 md:grid-cols-4"><span className="min-w-0 truncate">Temp {review.temperature_rating ?? "—"}/5</span><span className="min-w-0 truncate">Spice {review.spiciness_rating ?? "—"}/5</span><span className="min-w-0 truncate">Sweet {review.sweet_savory_rating ?? "—"}/5</span><span className="min-w-0 truncate">Flavor {review.flavor_intensity_rating ?? "—"}/5</span></div>{review.review && <p className="mt-2 text-sm text-secondary">{review.review}</p>}</article>) : <p className="text-sm font-semibold text-secondary">No reviews yet.</p>}</section>;
};
...
const DashboardDishGrid = ({ title, dishes, empty }: { title: string; dishes: DashboardDish[]; empty: string }) => <section className="max-w-full overflow-hidden rounded-[28px] glass-surface p-4"><div className="mb-3 flex min-w-0 items-center justify-between gap-3"><h2 className="min-w-0 truncate font-display text-2xl font-black">{title}</h2><span className="soft-chip">{dishes.length}</span></div>{dishes.length ? <div className="grid min-w-0 grid-cols-2 gap-3 md:grid-cols-3">{dishes.slice(0, 6).map((dish) => <a key={`${title}-${dish.id}`} href={`/dish/${dish.slug}`} className="group min-w-0 overflow-hidden rounded-3xl bg-secondary/70 shadow-[var(--shadow-soft)] ring-1 ring-border/50 transition active:scale-[0.98]"><div className="relative aspect-[4/5] overflow-hidden bg-secondary">{dish.cover_image_url ? <img src={dish.cover_image_url} alt={dish.name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" decoding="async" sizes="(min-width: 768px) 33vw, 50vw" /> : <div className="flex h-full items-center justify-center"><ChefHat className="size-10 opacity-40" /></div>}<div className="absolute inset-0 bg-gradient-to-t from-foreground/90 via-foreground/38 to-transparent" /><span className="absolute left-2 top-2 soft-chip text-primary"><Star className="size-3 fill-current" />{(dish.user_rating ?? dish.aggregate_rating ?? 0).toFixed(1)}</span><div className="absolute inset-x-0 bottom-0 p-3"><p className="line-clamp-2 font-display text-xl font-black leading-none">{dish.name}</p><p className="mt-1 line-clamp-1 text-xs font-bold text-secondary">{dish.restaurant_name || dish.cuisine || dish.section || "Dish"}</p></div></div></a>)}</div> : <p className="rounded-3xl bg-secondary/60 p-4 text-sm font-bold text-secondary">{empty}</p>}</section>;

const CollectionGrid = ({ collections }: { collections: CollectionPreview[] }) => <section className="max-w-full overflow-hidden rounded-[28px] glass-surface p-4"><div className="mb-3 flex min-w-0 items-center justify-between gap-3"><h2 className="min-w-0 truncate font-display text-2xl font-black">Collections</h2><span className="soft-chip">{collections.length}</span></div>{collections.length ? <div className="grid min-w-0 gap-3 md:grid-cols-2">{collections.map((collection) => <article key={collection.id} className="min-w-0 overflow-hidden rounded-3xl bg-secondary/65 p-3 ring-1 ring-border/50"><div className="grid aspect-[5/3] grid-cols-3 gap-1 overflow-hidden rounded-2xl bg-secondary">{collection.dishes.slice(0, 3).map((dish) => <a key={dish.id} href={`/dish/${dish.slug}`} className="min-w-0 overflow-hidden bg-card/70">{dish.cover_image_url ? <img src={dish.cover_image_url} alt={dish.name} className="h-full w-full object-cover" loading="lazy" decoding="async" /> : <div className="flex h-full items-center justify-center"><ChefHat className="size-6 opacity-40" /></div>}</a>)}{!collection.dishes.length && <div className="col-span-3 flex h-full items-center justify-center"><Bookmark className="size-10 opacity-40" /></div>}</div><div className="mt-3 flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-display text-xl font-black">{collection.name}</h3><p className="mt-1 text-xs font-bold text-primary">{collection.item_count ?? collection.dishes.length} dishes · {collection.is_public ? "Public" : "Private"}</p></div></div>{collection.description && <p className="mt-2 line-clamp-2 text-sm text-secondary">{collection.description}</p>}</article>)}</div> : <p className="rounded-3xl bg-secondary/60 p-4 text-sm font-bold text-secondary">Saved dishes land in Saved. Create collections like NYC spots, Best desserts, or Date night.</p>}</section>;
...
const WantToTryPlanner = ({ dishes, hasLocation, onUseLocation }: { dishes: WantToTryDish[]; hasLocation: boolean; onUseLocation: () => void }) => {
  ...
  return <section className="max-w-full overflow-hidden rounded-[28px] glass-surface p-4"><div className="mb-3 flex min-w-0 flex-wrap items-center justify-between gap-3"><div><h2 className="font-display text-2xl font-black">Want to try</h2>{nearest && <p className="text-xs font-bold text-secondary">Nearest: {nearest.name} · {nearest.distance_miles?.toFixed(1)} mi</p>}</div><div className="flex items-center gap-2"><span className="soft-chip">{dishes.length}</span>{!hasLocation && <Button size="sm" variant="outline" className="rounded-full" onClick={onUseLocation}><LocateFixed className="size-4" />Sort nearby</Button>}</div></div>{dishes.length ? <div className="space-y-4">{groups.map(([location, items]) => <div key={location} className="rounded-3xl bg-secondary/55 p-3"><div className="mb-3 flex min-w-0 items-center justify-between gap-2"><h3 className="min-w-0 break-words font-display text-xl font-black"><MapPin className="mr-1 inline size-4 text-primary" />{location}</h3><span className="soft-chip text-primary">{items.length}</span></div><div className="grid min-w-0 gap-3 md:grid-cols-2">{items.map((dish) => <a key={dish.id} href={`/dish/${dish.slug}`} className="group grid min-w-0 grid-cols-[72px_minmax(0,1fr)] gap-3 rounded-2xl bg-card/95 p-2 ring-1 ring-border/45 transition active:scale-[0.98] sm:grid-cols-[84px_minmax(0,1fr)]"><div className="aspect-square overflow-hidden rounded-2xl bg-secondary">{dish.cover_image_url ? <img src={dish.cover_image_url} alt={dish.name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" decoding="async" sizes="(min-width: 768px) 33vw, 50vw" /> : <div className="flex h-full items-center justify-center"><ChefHat className="size-8 opacity-40" /></div>}</div><div className="min-w-0 py-1"><div className="mb-1 flex min-w-0 flex-wrap gap-1"><span className="soft-chip text-primary"><Star className="size-3 fill-current" />{Number(dish.aggregate_rating ?? 0).toFixed(1)}</span>{dish.distance_miles != null && <span className="soft-chip">{dish.distance_miles.toFixed(1)} mi</span>}</div><p className="line-clamp-2 font-display text-lg font-black leading-none">{dish.name}</p><p className="mt-1 line-clamp-1 text-xs font-bold text-secondary">{dish.restaurants?.name || dish.cuisine || "Dish"}</p><p className="mt-2 text-xs font-black text-primary">{dish.plan_group}</p></div></a>)}</div></div>)}</div> : <p className="rounded-3xl bg-secondary/60 p-4 text-sm font-bold text-secondary">Save dishes you are craving next.</p>}</section>;
};
...
const ProfilePanel = ({ sessionUser, userLocation, onUseLocation, onProtected }: { sessionUser: UserSession; userLocation: { latitude: number; longitude: number } | null; onUseLocation: () => void; onProtected: (message: string) => void }) => {
  ...
  if (!sessionUser) return <section className="rounded-[28px] glass-surface p-5"><h1 className="font-display text-4xl font-black">Your food identity</h1><p className="mt-2 text-secondary">Browse, search, take photos, and draft reviews. Sign in when you want to save your taste.</p><Button className="mt-4 rounded-full" onClick={() => onProtected("Sign in to save favorites, lists, and contribution history.")}><LogIn />Sign in</Button></section>;
  return <section className="space-y-4"><div className="relative overflow-hidden rounded-[32px] bg-secondary p-5 shadow-[var(--shadow-editorial)] ring-1 ring-border/60"><div className="absolute inset-0 bg-gradient-to-br from-primary/20 via-transparent to-accent/15" /><div className="relative"><p className="soft-chip mb-3 text-primary"><User className="size-4" />Food profile</p><h1 className="break-words font-display text-4xl font-black leading-none sm:text-5xl">Your taste map</h1><p className="mt-2 line-clamp-1 text-sm font-bold text-secondary">{sessionUser.email}</p><div className="mt-5 grid grid-cols-3 gap-2"><Metric icon={Heart} label="favorites" value={String(favorites.length)} /><Metric icon={Bookmark} label="want" value={String(wantToTry.length)} /><Metric icon={Star} label="avg" value={averageRating} /></div></div></div>{loading ? <SearchResultsLoader /> : <><CollectionGrid collections={collections} /><ShareableLists sessionUser={sessionUser} onProtected={onProtected} /><DashboardDishGrid title="Favorite dishes" dishes={favorites} empty="Favorite dishes to shape your identity." /><WantToTryPlanner hasLocation={Boolean(userLocation)} onUseLocation={onUseLocation} dishes={wantToTryPlan.length ? wantToTryPlan : wantToTry.map((dish) => ({ ...dish, tags: [], dietary_tags: [], currency: "USD", photo_count: 0, restaurants: dish.restaurant_name ? { name: dish.restaurant_name } : null, location_group: dish.restaurant_name || "Location pending", plan_group: "Saved for later" as const }))} /><DashboardDishGrid title="Recently rated" dishes={recentRated} empty="Rate dishes to build your taste history." /><div className="grid gap-4 md:grid-cols-2"><section className="rounded-[28px] glass-surface p-4"><h2 className="font-display text-2xl font-black">Top cuisines</h2><div className="mt-3 flex flex-wrap gap-2">{topCuisines.length ? topCuisines.map(([name, count]) => <span key={name} className="soft-chip text-primary">{name} · {count}</span>) : <p className="text-sm font-bold text-secondary">Start saving dishes to reveal your cuisine pattern.</p>}</div></section><section className="rounded-[28px] glass-surface p-4"><h2 className="font-display text-2xl font-black">Most visited</h2><div className="mt-3 space-y-2">{topRestaurants.length ? topRestaurants.map(([name, count]) => <div key={name} className="flex items-center justify-between rounded-2xl bg-secondary/70 p-3"><span className="font-black line-clamp-1">{name}</span><span className="soft-chip">{count}</span></div>) : <p className="text-sm font-bold text-secondary">Restaurant patterns appear as you rate and save dishes.</p>}</div></section></div></>}</section>;
};
