...
const FeedItemCard = ({ item, userLocation, onSave, onFirstReview, onDishAction }: { item: MenuItem; userLocation: { latitude: number; longitude: number } | null; onSave: (item: MenuItem) => void; onFirstReview?: (item: MenuItem) => void; onDishAction?: (item: MenuItem, action: "want_to_try" | "favorite", enabled: boolean) => void }) => {
  ...
  return (
    <article className="feed-reel group relative -mx-3 min-h-[calc(100svh-148px)] overflow-hidden bg-secondary shadow-[var(--shadow-editorial)] ring-1 ring-border/55 md:mx-0 md:min-h-[760px] md:rounded-[32px]">
      ...
      <div className="pointer-events-none absolute left-4 top-4 flex flex-wrap gap-2"><span className="soft-chip text-accent"><Star className="size-4 fill-current" />{item.aggregate_rating.toFixed(1)}</span>{trendLabel && <span className="soft-chip text-accent"><Sparkles className="size-4" />{trendLabel}</span>}</div>
      ...
      <section className="mx-auto grid max-w-5xl gap-5 px-0 py-3 md:grid-cols-[180px_1fr] md:px-6 md:py-6">
        ...
        <div key={`${view}-${location.pathname}`} className="screen-enter min-w-0 space-y-4 px-3 md:px-0">
          ...
          {view === "discover" && !selectedItem && !listSlug && (
            <>
              <section className="sticky top-[66px] z-20 -mx-3 space-y-2 border-y border-border/40 bg-background/72 px-3 py-2 backdrop-blur-2xl md:top-20 md:mx-0 md:rounded-[28px] md:border">
                ...
                <div className="flex gap-2 overflow-x-auto pb-1">{suggestedSearches.map((suggestion) => <button key={suggestion} type="button" onClick={() => applySearchSuggestion(suggestion)} className="soft-chip shrink-0 transition active:scale-95">{suggestion}</button>)}</div>
              ...
              {!query && <div className="flex gap-2 overflow-x-auto pb-1">{trendingQueries.map((trend) => <button key={trend} type="button" onClick={() => applySearchSuggestion(trend)} className="soft-chip shrink-0 text-accent"><Sparkles className="size-4" />{trend}</button>)}</div>}
  ...
  return (
    <section className="-mx-3 space-y-4 md:mx-0">
      <div className="relative min-h-[calc(100svh-108px)] overflow-hidden bg-secondary shadow-[var(--shadow-editorial)] ring-1 ring-border/60 md:min-h-[760px] md:rounded-[32px]">
        ...
        <div className="absolute left-4 right-4 top-4 flex items-center justify-between gap-2"><div className="flex min-w-0 gap-2">{isTrending ? <span className="soft-chip text-accent"><Sparkles className="size-4" />{trendLabel ?? "Trending"}</span> : <span className="soft-chip">{item.cuisine || item.section || "Dish"}</span>}</div><div className="flex gap-2"><Button size="icon" variant="secondary" className="size-11 rounded-full bg-background/70 backdrop-blur-xl" onClick={copyLink} aria-label="Copy dish link"><Copy className="size-5" /></Button><Button size="icon" variant="secondary" className="size-11 rounded-full bg-background/70 backdrop-blur-xl" onClick={shareItem} aria-label="Share dish"><Share2 className="size-5" /></Button></div></div>
...
const ProfilePanel = ({ sessionUser, onProtected }: { sessionUser: UserSession; onProtected: (message: string) => void }) => {
  ...
  useEffect(() => {
    ...
    ]).then(async ([savedResult, ratingsResult]) => {
