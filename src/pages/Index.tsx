          ...
          {view === "discover" && !selectedItem && !listSlug && (
            <>
              <section className="sticky top-[66px] z-20 -mx-3 max-w-[calc(100%+1.5rem)] space-y-2 overflow-hidden border-y border-border/40 bg-background/72 px-3 py-2 backdrop-blur-2xl md:top-20 md:mx-0 md:max-w-full md:rounded-[28px] md:border">
                ...
                <div className="flex max-w-full min-w-0 gap-2 overflow-x-auto overscroll-x-contain pb-1">{suggestedSearches.map((suggestion) => <button key={suggestion} type="button" onClick={() => applySearchSuggestion(suggestion)} className="soft-chip shrink-0 transition active:scale-95">{suggestion}</button>)}</div>
              ...
              {!query && <div className="flex max-w-full min-w-0 gap-2 overflow-x-auto overscroll-x-contain pb-1">{trendingQueries.map((trend) => <button key={trend} type="button" onClick={() => applySearchSuggestion(trend)} className="soft-chip shrink-0 text-accent"><Sparkles className="size-4" />{trend}</button>)}</div>}
